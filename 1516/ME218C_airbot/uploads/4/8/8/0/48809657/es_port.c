...
void interrupt ISR ( void )
{
   ...
   if (IOCIF == 1) {
       AccelerometerIOC = Accel_IOC_ISR();
   }
...
bool _HW_Process_Pending_Ints( void )
{
   ...
   if (AccelerometerIOC) {
       ES_Event TriggerEvent;
       TriggerEvent.EventType = PAIR_TRIGGER;
       PostPACService(TriggerEvent);
       AccelerometerIOC = false;
   }
